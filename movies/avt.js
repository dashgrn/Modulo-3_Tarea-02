...
function cardPopulator(movieData) {
    const index = movieData.findIndex(function (movie) { return movie.id === 'avt'});
    movieTitle.textContent = movieData[index].title;
    movieImg.setAttribute('src', movieData[index].img);
    movieScore.textContent = `Rating: ${movieData[index].score}`;
    movieSinopsis.textContent = movieData[index].sinopsis;
}
